...
def img2label_paths(img_paths):
    ...
    sa, sb, sc = f'{os.sep}images{os.sep}', f'{os.sep}labels{os.sep}', f'{os.sep}原始库{os.sep}'  # /images/, /labels/ substrings
    return [sb.join(sb.join(x.rsplit(sa, 1)).rsplit(sc, 1)).rsplit('.', 1)[0] + '.txt' for x in img_paths]
...
class LoadImagesAndLabels(Dataset):
    ...
    def load_mosaic(self, index):
        ...
        indices = [index] + random.choices(self.indices, k=3)  # 3 additional image indices 涓夊紶鍥剧墖鍙兘灞炰簬鍚屼竴寮犲浘鐗?        random.shuffle(indices)
        for i, index in enumerate(indices):
            # Load image
            img, _, (h, w) = self.load_image(index) #h,w:after resize
        ...
        # Concat/clip labels
        labels4 = np.concatenate(labels4, 0)#鎸夊垪鎷兼帴
