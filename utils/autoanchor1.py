...
def kmean_anchors(dataset='./data/coco128.yaml', n=9, img_size=640, thr=4.0, gen=1000, verbose=True):
    """ Creates kmeans-evolved anchors from training dataset with K-Means++ initialization
    ...
    def kmeans_pp_init(data, k):
        ...
        for _ in range(1, k):
            ...
            dists = pairwise_distances(data, np.array(centers), metric='euclidean')
            min_dists = np.min(dists, axis=1)
            
            # Compute probabilities
            probs = min_dists**2 / np.sum(min_dists**2)
    ...
    try:
        ...
        # K-means++ initialization
        initial_centers = kmeans_pp_init(wh_normalized, n)
        
        # Run standard k-means with K-means++ initialization
        k = kmeans(wh_normalized, initial_centers, iter=30)[0] * s  # points
