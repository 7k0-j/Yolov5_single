...
def check_git_status(repo='ultralytics/yolov5'):
    ...
    if n > 0:
        ...
        s += f"鈿狅笍 YOLOv5 is out of date by {n} commit{'s' * (n > 1)}. Use `{pull}` or `git clone {url}` to update."
    else:
...
def check_dataset(data, autodownload=True):
    ...
    if 'names' not in data:
        LOGGER.warning("data.yaml 'names:' field missing 鈿狅笍, assigning default names 'class0', 'class1', etc.")
    ...
    if val:
        ...
        if not all(x.exists() for x in val):
            LOGGER.info('\nDataset not found 鈿狅笍, missing paths %s' % [str(x) for x in val if not x.exists()])
            ...
            s = f"success 鉁?{dt}, saved to {colorstr('bold', root)}" if r in (0, None) else f"failure {dt} ?"
...
def check_amp(model):
    ...
    try:
        ...
        LOGGER.info(f'{prefix}checks passed 鉁?')
        return True
    except Exception:
        help_url = 'https://github.com/ultralytics/yolov5/issues/7908'
        LOGGER.warning(f'{prefix}checks failed 鉂? disabling Automatic Mixed Precision. See {help_url}')
        return False
...
def DIoU_NMS(boxes, scores, iou_thres, GIoU=False, DIoU=False, CIoU=False):
    """
...
def non_max_suppression(prediction,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        #i = DIoU_NMS(boxes, scores, iou_thres, GIoU=False, DIoU=True, CIoU=False) #DIOU_NMS
